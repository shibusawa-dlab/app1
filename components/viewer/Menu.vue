  ...
  <v-list>
    ...
    <template v-for="(item, key) in items">
      ...
      <template v-if="item.children">
        <v-list-group :key="key" no-action sub-group>
          <template #activator>
            <v-list-item-content>
              <!--  @click="id = item.id" -->
          ...
          <template v-for="(item2, key2) in item.children">
            <template v-if="item2.children">
              <v-list-group :key="key2" no-action sub-group>
                <template #activator>
                  <v-list-item-content>
                    <!--  @click="id = item2.id" -->
      ...
      <template v-else>
        <v-list-item :key="key" :disabled="item.disabled" @click="id = item.id">
          <v-list-item-title>{{ item.label }}</v-list-item-title>
        </v-list-item>
      </template>
    </template>
...
export default class Menu extends Vue {
  ...
  aaa(element: any) {
    ...
    for (const child of childrenOfFront) {
      if (
        child.hasAttribute('xml:id') &&
        $(child).find('tei-head').length > 0
      ) {
        menu.push({
