<template>
  ...
  <div slot-scope="{ currentRefinement, refine }">
    ...
    :placeholder="$t('add_a_search_term')"
            ...
            @click:append="search(currentRefinement, refine, $event)"
            @keydown.enter="search(currentRefinement, refine, $event)"
          ></v-text-field>
  </div>
</ais-search-box>
        ...
        <v-row class="mt-10" dense>
          ...
          <v-col v-show="false" cols="12" sm="2"
            ><ais-sort-by
              :items="[{ value: 'dev_MAIN', label: this.$t('relevance') }]"
            >
              <v-select
                v-model="sort"
          ...
          <v-col col="12" sm="4" order-sm="1">
            <ais-clear-refinements>
              <div slot-scope="{ canRefine, refine, createURL }">
                <div v-if="canRefine" class="mb-4 text-right">
                  <small>
...
export default {
  ...
  methods: {
    search(currentRefinement, refine){
